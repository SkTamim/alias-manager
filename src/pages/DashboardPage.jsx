...
export default function DashboardPage() {
    ...
    const handleSaveAlias = async (aliasData) => { const { id, ...data } = aliasData; try { if (id) { await updateDoc(doc(db, 'users', currentUser.uid, 'aliases', id), data); } else { await addDoc(getCollectionRef(), data); } } catch (error) { console.error("Error saving alias:", error); setAlertProps({ isOpen: true, title: "Save Error", message: "Could not save the alias. Please try again." }); } setIsModalOpen(false); setEditingAlias(null); };
    const handleDeleteAlias = (id, key) => { setConfirmProps({ isOpen: true, title: 'Delete Alias', message: `Are you sure you want to delete the alias "${key}"? This action cannot be undone.`, onConfirm: async () => { try { await deleteDoc(doc(db, 'users', currentUser.uid, 'aliases', id)); } catch (error) { console.error("Error deleting alias:", error); setAlertProps({ isOpen: true, title: "Delete Error", message: "Could not delete the alias. Please try again." }); } } }); };
    const handleExportTxt = () => { if (aliases.length === 0) { setAlertProps({ isOpen: true, title: "Export Empty", message: "There are no aliases to export." }); return; } const content = aliases.map(a => `${a.key} "${a.command}"`).join('\n'); const blob = new Blob([content], { type: 'text/plain;charset=utf-8' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'Rhino_Aliases.txt'; document.body.appendChild(a); a.click(); document.body.removeChild(a); };
    const handleFileImport = (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (event) => { const content = event.target.result; const parsedAliases = []; content.split(/\r?\n/).forEach(line => { line = line.trim(); if (!line || line.startsWith("'")) return; const parts = line.split(/\s+/); const key = parts[0]; let command = parts.slice(1).join(' ').replace(/"/g, ''); let description = ''; const simpleCommand = command.toLowerCase().replace(/[^a-z0-9]/g, ''); if (commandDictionary[simpleCommand]) { command = commandDictionary[simpleCommand].command; description = commandDictionary[simpleCommand].description; } else if (!command.startsWith("! _")) { command = `! _${command}`; } if(key) parsedAliases.push({ key, command, description }); }); if (parsedAliases.length > 0) { setConfirmProps({ isOpen: true, title: 'Import Aliases', message: `Found ${parsedAliases.length} aliases. This will ADD them to your current list. Do you want to continue?`, onConfirm: async () => { try { const batch = writeBatch(db); parsedAliases.forEach(alias => { const newDocRef = doc(getCollectionRef()); batch.set(newDocRef, alias); }); await batch.commit(); setAlertProps({ isOpen: true, title: "Import Successful", message: `${parsedAliases.length} aliases have been added to your dashboard.` }); } catch (error) { console.error("Error during batch import:", error); setAlertProps({ isOpen: true, title: "Import Failed", message: "Could not import aliases. Please check the console." }); } } }); } else { setAlertProps({ isOpen: true, title: "Import Failed", message: "Could not find any valid aliases in the selected file." }); } }; reader.readAsText(file); e.target.value = null; };
                                        ...
                                        {filteredAliases.map((alias) => (
                                            <tr key={alias.id} className="transition-colors hover:bg-gray-800">
                                                ...
                                                <td className="px-6 py-4 text-right pr-6">
                                                    {/* UPDATED: Buttons are now always visible */}
                                                    <div className="flex justify-end items-center gap-2">
                                                        <button onClick={() => { setEditingAlias(alias); setIsModalOpen(true); }} className="p-2 rounded-md text-gray-400 hover:text-yellow-400 hover:bg-gray-700 transition-colors" title="Edit Alias">
                                                            <PencilIcon />
                                                        </button>
                                                        <button onClick={() => handleDeleteAlias(alias.id, alias.key)} className="p-2 rounded-md text-gray-400 hover:text-red-500 hover:bg-gray-700 transition-colors" title="Delete Alias">
                                                            <TrashIcon />
                                                        </button>
                                                    </div>
                                                </td>
